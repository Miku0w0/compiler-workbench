 ...
 * stack.cl
 *
 * 一个用 COOL 语言实现的通用栈数据结构。
 * 这个栈可以存储任何继承自 Object 的类型的元素，
 * 但为了演示，我们主要关注 Int 和 String。
 *
 *)
...
class StackNode inherits Object {
    item : Object;     -- 节点存储的数据，类型为 Object 使其通用
...
(*
 * Stack 类
 * 实现了栈的核心功能。内部使用 StackNode 构成的链表来存储数据。
 * 栈顶由属性 top 表示。
 *)
class Stack inherits IO {
        ...
        if isEmpty() then
            {
                ...
                abort();
                new Object;  -- abort() 会中止程序，这行为是为了让类型检查器满意
            }
        else
            top.getItem()
        fi
    ...
    print() : SELF_TYPE {
        {  -- 这里是修正的关键：添加了起始花括号
            if isEmpty() then
            ...
            else
                {
                    ...
                    while not (isvoid current) loop
                        {
                            -- 使用 case 语句判断元素的具体类型，并调用合适的方法打印
            ...
            self;  -- self 作为整个块的返回值，类型是 SELF_TYPE，符合方法要求
        }  -- <---这里是修正的关键：添加了结束花括号
    };
};
